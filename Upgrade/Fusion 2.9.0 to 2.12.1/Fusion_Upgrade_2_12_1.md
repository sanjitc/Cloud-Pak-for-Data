...
### Step 3: Verify/Install Required Operator Packages

#### Check for Existing Red Hat Operator Packages

Run the following command to verify if `redhat-oadp-operator` and `amq-streams` are already mirrored:

```bash
oc get packagemanifests | grep -i "Red Hat Operator"
```

#### Mirror Red Hat Operators (if not already present)

If you have not previously mirrored `redhat-oadp-operator` and `amq-streams` from Red Hat packages, follow the steps in the [Mirroring Red Hat operator images to enterprise registry](https://www.ibm.com/docs/en/fusion-software/2.12.x?topic=registry-mirroring-red-hat-operator-images-enterprise) documentation.

> ⚠️ **Important:** If you skip this step and these operators are not available, old packages will be lost from the Red Hat operator index image and the ImageSetConfiguration file will not be updated.

### Step 4: Configure ibm-pak Plugin
...
### Step 5: Download Mirroring Metadata
...
Creating inventory ...
- Success
CASE: ibm-spectrum-fusion-sds, version: 2.12.1 downloaded to ~/.ibm-pak
```

### Step 6: Generate oc-mirror Configuration Files
...
- Success
Generating catalog sources ...
- Success
```

> 📝 **Note:** Use the `--dest-tls-verify=false` parameter when mirroring images to a quay repository.

### Step 7: Run the oc-mirror Command
...
**Expected Output on Successful Mirroring:**
```
...
> ⚠️ **Important:** In case of any failure, refer to the [Troubleshooting and known issues in offline mirroring](https://www.ibm.com/docs/en/fusion-software/2.12.x?topic=mirroring-troubleshooting-known-issues-in-offline) documentation.

### Step 8: Navigate to Generated Files Directory
...
### Step 9: Apply Generated Configuration Files
...
> 📝 **Note:** Catalogs with `-linux-amd64` in the name contain multi-architecture images and are not limited to AMD64. A nodeSelector set to `kubernetes.io/arch: amd64` does not restrict the catalog to AMD64 images. The catalog includes multi-architecture images and can be applied without modification.

### Step 10: Clean Up Additional Catalog Sources
...
Check that nodes have been updated with the new mirror configuration:
...
#### Issue 1: Mirror Command Fails
**Symptoms:** `oc mirror` command fails with connection errors

**Resolution:**
1. Verify network connectivity to IBM CloudPak repository
2. Check firewall rules
3. Verify credentials for enterprise registry
4. Review logs in `oc-mirror-workspace/` directory

#### Issue 2: Operator Packages Not Found
**Symptoms:** Missing `redhat-oadp-operator` or `amq-streams` packages

**Resolution:**
1. Follow the Red Hat operator mirroring procedure
2. Verify Red Hat operator catalog is accessible
3. Check package manifests: `oc get packagemanifests`

#### Issue 3: CatalogSource Not Ready
**Symptoms:** CatalogSource shows "NOT READY" status
...
oc get secrets -n openshift-marketplace
```

#### Issue 4: Image Pull Errors
**Symptoms:** Pods fail to pull images from enterprise registry

**Resolution:**
1. Verify ImageDigestMirrorSet is applied correctly
2. Check registry credentials
3. Verify image paths in enterprise registry
4. Check node configuration for registry mirrors

#### Issue 5: Insufficient Storage
**Symptoms:** Mirror process fails due to disk space

**Resolution:**
1. Check available disk space: `df -h`
2. Clean up old images if necessary
3. Ensure enterprise registry has sufficient storage

### Getting Additional Help

For detailed troubleshooting steps, refer to:
- [Troubleshooting and known issues in offline mirroring](https://www.ibm.com/docs/en/fusion-software/2.12.x?topic=mirroring-troubleshooting-known-issues-in-offline)
- IBM Support Portal
...
- [ ] Capture current state logs and configurations
- [ ] Notify stakeholders

### Rollback Steps

1. **Revert CatalogSource:**
```bash
# Remove new catalog sources
oc delete -f catalog-sources-linux-amd64.yaml

# Restore previous catalog sources (if backed up)
oc apply -f <backup-catalog-sources.yaml>
```

2. **Revert ImageDigestMirrorSet:**
```bash
# Remove new mirror configuration
oc delete -f image-digest-mirror-set.yaml

# Restore previous mirror configuration (if backed up)
oc apply -f <backup-image-digest-mirror-set.yaml>
```
...
oc get packagemanifests | grep fusion
...
4. **Clean Up:**
```bash
# Remove downloaded CASE files if needed
rm -rf ~/.ibm-pak/data/cases/${CASE_NAME}/${CASE_VERSION}
```
