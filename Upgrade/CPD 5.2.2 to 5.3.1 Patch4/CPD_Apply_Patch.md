# Applying patches to Software Hub 5.3.1
## Context
- **OCP:** 4.16
- **CPD:** 5.3.1 Patch 3 -> Patch 6
- **Storage:** ODF
...
# Pre-requisites
### 1. Backup of the cluster 
Backup your Cloud Pak for Data cluster before applying patches.

### 2. The permissions required
- Cluster administrator 
- Instance administrator 
- Registry administrator

### 3. Setting up a client workstation
...
### 4. Gathering information about installed components
```
${CPDM_OC_LOGIN}
...
```
>[!IMPORTANT]
>Copy the `case-download` command that was returned by the `list-patch` command.

### 6. Updating your environment variables script
...
### 7. Refresh the olm-utils container
...
### 8. Downloading CASE packages and cluster-scoped resource files 
#### 8.1. Downloading CASE packages
The `case-download` command that was generated by the `list-patch` command ([Step 5 above](https://github.com/sanjitc/Cloud-Pak-for-Data/blob/main/Upgrade/CPD%205.2.2%20to%205.3.1%20Patch4/CPD_Apply_Patch.md#5-checking-for-new-patches)) assumes that you are downloading the CASE packages from GitHub.
```
...
--patch_id=${PATCH_ID}
```
#### 8.2. Downloading cluster-scoped resource files
