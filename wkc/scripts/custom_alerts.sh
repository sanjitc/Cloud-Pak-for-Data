...
couchdb_designdoc_reindexing() {
  oc exec wdp-couchdb-0 -c couchdb -- bash -c 'curl -ks -u "admin:`cat /etc/.secrets/COUCHDB_PASSWORD`" https://localhost:6984/_active_tasks' |jq '.[] | select((.type == "search_indexer") and (.design_document|endswith("_temporary")))' > /tmp/couchdb_designdoc_reindexing.out

  if [ `stat -c %s /tmp/couchdb_designdoc_reindexing.out` -eq 0 ]; then
	  echo "file empty"
